...
def expand(inputs,dur):
    """

    :param inputs: [N,T_x,hidden]
    :param dur:    [N,T_x]
    :return:
    """

    N,T_x,_=inputs.shape
    frames=[]
    if inputs.shape[1]!=dur.shape[1]:
        print(inputs.shape)
        print(dur.shape)
        raise RuntimeError
    for i in range(N):
        frame = []
        for j in range(T_x):
            frame.append(np.tile(inputs[i,j,:],reps=[1,dur[i,j],1]))
        frame=np.concatenate(frame,axis=1)
        frames.append(frame)
    pad=max([frame.shape[1] for frame in frames])

    for i in range(len(frames)):
        frames[i]=np.pad(frames[i],[(0,0),(0,pad-(frames[i].shape[1])),(0,0)],mode='constant',constant_values=0)   #[1,max_Ty,hidden]
    frames=np.concatenate(frames,axis=0)   #[N,max_Ty,hidden]

    return frames.astype(np.float32),np.sum(dur,axis=-1).astype(np.int32)

def length_regulator(inputs, dur):
    Batch_size = tf.shape(inputs)[0]
    d_model = tf.shape(inputs)[-1]
    max_frame = tf.reduce_max(tf.reduce_sum(dur, axis=-1))
    decoder_inputs, T_y = tf.py_func(expand, [inputs, dur], [tf.float32, tf.int32])
    decoder_inputs = tf.reshape(decoder_inputs, [Batch_size, max_frame, d_model])
    T_y = tf.reshape(T_y, [Batch_size, 1])

    return decoder_inputs, T_y


def np_positional_encoding(max_len, d_model):
    pos_enc = np.asarray(
        [[position / np.power(1000, 2 * i / d_model) for i in range(d_model)] for position in range(max_len)])
...
def positional_encoding(inputs, T):
    with tf.variable_scope('positional_encoding') as scope:
        d_model = tf.shape(inputs)[-1]
        batch = tf.shape(inputs)[0]
        max_len = tf.shape(inputs)[1]
        pos_enc = tf.py_func(np_positional_encoding, [max_len, d_model], [tf.float32])[0]
        index = tf.cast(tf.tile(tf.expand_dims(tf.range(max_len), axis=0), [batch, 1]), tf.int32)
        output = tf.nn.embedding_lookup(pos_enc, index)
        return output
