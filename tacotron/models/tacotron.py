...
def split_input_func(x, split_pos):
	rst = []
	start = 0
	# x will be a numpy array with the contents of the placeholder below
	for i in range(split_pos.shape[0]):
...
class Tacotron():
	...
	def initialize(self, inputs, input_lengths, mel_targets=None, stop_token_targets=None, linear_targets=None,
		...
		max_out_length = tf.cast(tf.reduce_max(input_lengths), tf.int32)
		...
		with tf.device(split_device):
			...
			p_dur_targets = tf.py_func(split_func, [dur_targets, split_infos[:, 4]], lout_float) if dur_targets is not None else dur_targets
			p_alignment_targets = tf.py_func(split_func, [alignment_targets, split_infos[:, 5]], lout_float) if alignment_targets is not None else alignment_targets
			...
			tower_dur_targets = []
			tower_alignment_targets = []

			batch_size = tf.shape(inputs)[0]
			mel_channels = hp.num_mels
			linear_channels = hp.num_freq
			for i in range(hp.tacotron_num_gpus):
				tower_inputs.append(tf.reshape(p_inputs[i], [batch_size, -1, 2]))
				...
				if p_dur_targets is not None:
					tower_dur_targets.append(tf.reshape(p_dur_targets[i], [batch_size, -1, hp.dur_project_dim]))
				if p_alignment_targets is not None:
					tower_alignment_targets.append(tf.reshape(p_alignment_targets[i], [batch_size, -1, max_out_length]))
			...
			with tf.device(tf.train.replica_device_setter(ps_tasks=1, ps_device="/cpu:0", worker_device=gpus[i])):
				with tf.variable_scope('inference') as scope:
					...
					self.embedding_table = tf.get_variable(
						'inputs_embedding', [257, hp.embedding_dim], dtype=tf.float32)
					self.embedding_ryhthm = tf.get_variable(
						'ryhthm_embedding', [5, hp.embedding_rhythm], dtype=tf.float32)

					phone = tf.reshape(tf.slice(tower_inputs[i], [0, 0, 0], [-1, -1, 1]), [batch_size, -1])
					ryhthm = tf.reshape(tf.slice(tower_inputs[i], [0, 0, 1], [-1, -1, 1]), [batch_size, -1])
					embedded_phone = tf.nn.embedding_lookup(self.embedding_table, phone)
					embedded_ryhthm = tf.nn.embedding_lookup(self.embedding_ryhthm, ryhthm)
					embedded_inputs = tf.concat([embedded_phone, embedded_ryhthm], axis=-1)
					embedded_inputs = tf.layers.dense(inputs=embedded_inputs, units=hp.merge_dim)
					...
					if post_condition:
						...
						post_cbhg = CBHG(hp.cbhg_kernels, hp.cbhg_conv_channels, hp.cbhg_pool_size,
										 ...
										 hp.cbhg_highway_units, hp.cbhg_rnn_units, hp.batch_norm_position, is_training,
										 name='CBHG_postnet')
						...
						if hp.clip_outputs:
							linear_outputs = tf.minimum(
								tf.maximum(linear_outputs, T2_output_range[0] - hp.lower_bound_decay),
								T2_output_range[1])
		...
		self.tower_dur_target = tower_dur_targets
	...
	def add_loss(self):
				...
				with tf.variable_scope('loss') as scope:
					if hp.mask_decoder:
						...
						after = MaskedMSE(self.tower_mel_targets[i], self.tower_mel_outputs[i],
										  self.tower_targets_lengths[i],
										  hparams=self._hparams)
						dur = MaskedSigmoidCrossEntropy(self.tower_dur_target[i], self.tower_dur_output[i],
														self.tower_input_lengths[i])
						...
						dur = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=self.tower_dur_target[i],
																					 logits=self.tower_dur_output[i]))
						# Compute <stop_token> loss (for learning dynamic generation stop)
					...
					self.tower_dur_loss.append(dur)
		...
		self.dur_loss = total_dur_loss / hp.tacotron_num_gpus
